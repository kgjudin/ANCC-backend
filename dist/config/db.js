...
exports.pool = new Pool({
    connectionString: env_js_1.ENV.DATABASE_URL || 'postgresql://postgres:postgres@localhost:54322/postgres',
    max: 20,
...
exports.memoryStore = {
    ...
    roles: [
        {
            ...
            is_system: true,
            permissions: constants_1.ALL_PERMISSIONS.map((k) => ({
                key: k,
        ...
        },
        {
            ...
            is_system: true,
            permissions: constants_1.ALL_PERMISSIONS.filter((k) => ['employee', 'attendance', 'leave', 'holiday'].includes(k.split('.')[0])).map((k) => ({
                key: k,
                module: PERM_DESCRIPTIONS[k]?.module || k.split('.')[0],
                description: PERM_DESCRIPTIONS[k]?.description || k
            }))
        },
        {
            ...
            is_system: true,
            permissions: constants_1.ALL_PERMISSIONS.filter((k) => ['supplier', 'product', 'purchase', 'expense'].includes(k.split('.')[0])).map((k) => ({
                key: k,
                module: PERM_DESCRIPTIONS[k]?.module || k.split('.')[0],
                description: PERM_DESCRIPTIONS[k]?.description || k
            }))
        },
        {
            ...
            is_system: true,
            permissions: constants_1.ALL_PERMISSIONS.filter((k) => ['attendance.view', 'leave.view', 'leave.request'].includes(k)).map((k) => ({
                key: k,
                module: PERM_DESCRIPTIONS[k]?.module || k.split('.')[0],
                description: PERM_DESCRIPTIONS[k]?.description || k
            }))
    ...
    permissions: constants_1.ALL_PERMISSIONS.map((k, idx) => ({
        id: `perm-${idx}`,
        key: k,
        module: PERM_DESCRIPTIONS[k]?.module || k.split('.')[0],
        description: PERM_DESCRIPTIONS[k]?.description || k
...
function executeInMemoryQuery(text, params) {
    ...
    if (sql.includes('select count(*)')) {
        ...
        if (sql.includes('from employees'))
            countVal = exports.memoryStore.employees.length;
        else if (sql.includes('from attendance'))
            countVal = exports.memoryStore.attendance.length;
        else if (sql.includes('from leave_requests'))
            countVal = exports.memoryStore.leave_requests.length;
        else if (sql.includes('from suppliers'))
            countVal = exports.memoryStore.suppliers.length;
        else if (sql.includes('from purchases'))
            countVal = exports.memoryStore.purchases.length;
        else if (sql.includes('from products'))
            countVal = exports.memoryStore.products.length;
        else if (sql.includes('from expenses'))
            countVal = exports.memoryStore.expenses.length;
        else if (sql.includes('from sites'))
            countVal = exports.memoryStore.sites.length;
        return [{ total: countVal, cnt: countVal }];
    }
    if (sql.includes('sum(grand_total)')) {
        const totalP = exports.memoryStore.purchases.reduce((acc, x) => acc + (Number(x.grand_total) || 0), 0);
        const totalPaid = exports.memoryStore.purchases.reduce((acc, x) => acc + (Number(x.paid_amount) || 0), 0);
        const totalOut = exports.memoryStore.purchases.reduce((acc, x) => acc + (Number(x.outstanding_amount) || 0), 0);
        return [{
                total_purchases: totalP,
                total_paid: totalPaid,
                total_outstanding: totalOut,
                total_purchases_count: exports.memoryStore.purchases.length,
    ...
    if (sql.startsWith('insert into users')) {
        ...
        exports.memoryStore.users.push(newU);
        return [newU];
    }
    if (sql.startsWith('insert into user_roles')) {
        const userId = p[0];
        const roleId = p[1];
        const rObj = exports.memoryStore.roles.find((r) => r.id === roleId) || exports.memoryStore.roles[0];
        const emp = exports.memoryStore.employees.find((e) => e.user_id === userId);
        if (emp) {
            emp.role_id = rObj.id;
    ...
    if (sql.startsWith('insert into employees')) {
        ...
        exports.memoryStore.employees.unshift(newEmp);
        return [newEmp];
    }
    if (sql.startsWith('update employees')) {
        const targetId = p[7] || p[8];
        const emp = exports.memoryStore.employees.find((e) => e.id === targetId);
        if (emp) {
            if (p[0])
                emp.full_name = p[0];
            if (p[1])
    ...
    if (sql.startsWith('insert into roles')) {
        ...
        if (existingIdx >= 0) {
            exports.memoryStore.roles[existingIdx] = newRoleObj;
        }
        else {
            exports.memoryStore.roles.push(newRoleObj);
        }
        return [newRoleObj];
    }
    if (sql.startsWith('update roles')) {
        const targetRoleId = p[2] || p[3];
        const rObj = exports.memoryStore.roles.find((r) => r.id === targetRoleId);
        if (rObj) {
            if (p[0])
                rObj.name = p[0];
            if (p[1])
                rObj.description = p[1];
        }
        return rObj ? [rObj] : [];
    }
    // 4. Product & Inventory Mutations
    if (sql.startsWith('insert into products')) {
        const seq = exports.memoryStore.products.length + 1;
        ...
        exports.memoryStore.products.unshift(newProd);
        return [newProd];
    }
    if (sql.startsWith('update products')) {
        const targetId = p[6];
        const prod = exports.memoryStore.products.find((x) => x.id === targetId);
        if (prod) {
            prod.name = p[0];
            prod.category_id = p[1];
            const catObj = exports.memoryStore.product_categories.find((c) => c.id === p[1]);
    ...
    if (sql.startsWith('insert into suppliers')) {
        ...
        exports.memoryStore.suppliers.unshift(newSup);
        return [newSup];
    }
    if (sql.startsWith('insert into purchases')) {
        ...
        exports.memoryStore.purchases.unshift(newPur);
        return [newPur];
    }
    if (sql.startsWith('insert into purchase_items')) {
        ...
        exports.memoryStore.purchase_items.unshift(newItem);
        return [newItem];
    }
    if (sql.startsWith('insert into expenses')) {
        ...
        exports.memoryStore.expenses.unshift(newExp);
        return [newExp];
    }
    if (sql.startsWith('insert into attendance')) {
        ...
        if (existingIdx >= 0) {
            exports.memoryStore.attendance[existingIdx] = newAtt;
        }
        else {
            exports.memoryStore.attendance.unshift(newAtt);
        }
        return [newAtt];
    }
    if (sql.startsWith('insert into leave_requests')) {
        ...
        exports.memoryStore.leave_requests.unshift(newLeave);
        return [newLeave];
    }
    if (sql.startsWith('insert into holidays')) {
        ...
        exports.memoryStore.holidays.unshift(newHol);
        return [newHol];
    }
    if (sql.startsWith('insert into leave_types')) {
        ...
        exports.memoryStore.leave_types.unshift(newLt);
        return [newLt];
    }
    if (sql.startsWith('insert into audit_logs')) {
        ...
        exports.memoryStore.audit_logs.unshift(newAudit);
        return [newAudit];
    }
    ...
    if (sql.includes('from users')) {
        if (sql.includes('lower(') || sql.includes('where email')) {
            ...
            const matchedEmp = exports.memoryStore.employees.find((e) => e.email.toLowerCase() === searchEmail);
            if (matchedEmp) {
                return [{
        ...
        return exports.memoryStore.users.map((u) => {
            const emp = exports.memoryStore.employees.find((e) => e.user_id === u.id || e.email.toLowerCase() === u.email.toLowerCase());
            return {
                id: emp?.id || u.id,
                user_id: u.id,
                email: u.email,
    ...
    if (sql.includes('from employees')) {
        return exports.memoryStore.employees;
    }
    ...
    if (sql.includes('from permissions') || sql.includes('permissions p') || sql.includes('role_permissions')) {
        if (sql.includes('where ur.user_id') || sql.includes('ur.user_id = $1')) {
            const userId = p[0];
            const emp = exports.memoryStore.employees.find((e) => e.user_id === userId);
            const rObj = exports.memoryStore.roles.find((role) => role.id === emp?.role_id || role.name === emp?.role_name);
            const permList = rObj ? (rObj.permissions || []) : exports.memoryStore.permissions;
            return permList.map((pItem) => ({
    ...
    if (sql.includes('from roles') || (sql.includes('roles r') && !sql.includes('role_permissions'))) {
        if (sql.includes('where ur.user_id')) {
            const userId = p[0];
            const emp = exports.memoryStore.employees.find((e) => e.user_id === userId);
            const r = exports.memoryStore.roles.find((role) => role.name === emp?.role_name || role.id === emp?.role_id) || exports.memoryStore.roles[0];
            return [r];
        }
        return exports.memoryStore.roles.map((r) => ({
            ...r,
            permissions: (r.permissions || []).map((pItem) => {
    ...
    if (sql.includes('permissions')) {
        return exports.memoryStore.permissions;
    }
    if (sql.includes('from attendance')) {
        return exports.memoryStore.attendance;
    }
    if (sql.includes('from leave_types')) {
        return exports.memoryStore.leave_types;
    }
    if (sql.includes('from employee_leave_balances')) {
        return exports.memoryStore.employee_leave_balances;
    }
    if (sql.includes('from leave_requests')) {
        return exports.memoryStore.leave_requests;
    }
    if (sql.includes('from holidays')) {
        return exports.memoryStore.holidays;
    }
    if (sql.includes('from product_categories')) {
        return exports.memoryStore.product_categories;
    }
    if (sql.includes('from products')) {
        return exports.memoryStore.products;
    }
    if (sql.includes('from suppliers')) {
        return exports.memoryStore.suppliers;
    }
    if (sql.includes('from purchases')) {
        return exports.memoryStore.purchases;
    }
    if (sql.includes('from expenses')) {
        return exports.memoryStore.expenses;
    }
    ...
    if (sql.startsWith('insert into messages')) {
        ...
        exports.memoryStore.messages.push(newMsg);
        return [newMsg];
    }
    if (sql.includes('from chat_rooms')) {
        return exports.memoryStore.chat_rooms;
    }
    if (sql.includes('from messages')) {
        if (sql.includes('where room_id')) {
            const targetRoomId = p[0];
            return exports.memoryStore.messages.filter((m) => m.room_id === targetRoomId);
        }
        return exports.memoryStore.messages;
    }
    if (sql.includes('from audit_logs')) {
        return exports.memoryStore.audit_logs;
    }
    // Sites mutations and queries
    if (sql.startsWith('insert into sites')) {
        const seq = exports.memoryStore.sites.length + 1;
        ...
        exports.memoryStore.sites.unshift(newSite);
        return [newSite];
    }
    if (sql.startsWith('update sites')) {
        const targetId = p[4] || p[5];
        const site = exports.memoryStore.sites.find((s) => s.id === targetId);
        if (site) {
            if (p[0])
                site.name = p[0];
            if (p[1] !== undefined)
            ...
            if (p[2] !== undefined) {
                ...
                const empObj = exports.memoryStore.employees.find((e) => e.id === p[2]);
                site.site_manager_name = empObj?.full_name || undefined;
            }
            if (p[3])
                site.status = p[3];
            return [site];
    ...
    if (sql.startsWith('delete from sites')) {
        const targetId = p[0];
        const idx = exports.memoryStore.sites.findIndex((s) => s.id === targetId);
        if (idx >= 0) {
            const removed = exports.memoryStore.sites.splice(idx, 1);
            return removed;
        }
        return [];
    }
    if (sql.includes('from sites')) {
        if (sql.includes('where st.id = $1') || sql.includes('where id = $1')) {
            const targetId = p[0];
            const s = exports.memoryStore.sites.find((item) => item.id === targetId);
            return s ? [s] : [];
        }
        let list = [...exports.memoryStore.sites];
        if (p.length > 0) {
    ...
    if (sql.startsWith('insert into financial_documents')) {
        const siteObj = exports.memoryStore.sites.find((s) => s.id === p[3]);
        let parsedItems = [];
        try {
        ...
        exports.memoryStore.financial_documents.unshift(newDoc);
        return [newDoc];
    }
    if (sql.startsWith('update financial_documents')) {
        const targetId = p[3];
        const doc = exports.memoryStore.financial_documents.find((d) => d.id === targetId);
        if (doc) {
            if (p[0])
                doc.status = p[0];
            if (p[1] !== undefined)
                doc.admin_remarks = p[1];
            if (p[2])
                doc.approved_by = p[2];
            return [doc];
        }
    ...
    if (sql.includes('from financial_documents')) {
        if (sql.includes('where id = $1') || sql.includes('where fd.id = $1')) {
            const targetId = p[0];
            const match = (exports.memoryStore.financial_documents || []).find((d) => d.id === targetId);
            return match ? [match] : [];
        }
        return exports.memoryStore.financial_documents || [];
    }
    if (sql.includes('from product_requests')) {
        if (sql.includes('where id = $1') || sql.includes('where prq.id = $1')) {
            const targetId = p[0];
            const match = (exports.memoryStore.product_requests || []).filter((r) => r.id === targetId);
            return match;
        }
        return exports.memoryStore.product_requests || [];
    }
    if (sql.includes('from inventory_transactions')) {
        return exports.memoryStore.inventory_transactions || [];
    }
    if (sql.includes('from inventory')) {
        if (sql.includes('where site_id = $1 and product_id = $2')) {
            const siteId = p[0];
            const prodId = p[1];
            const match = (exports.memoryStore.inventory || []).find((inv) => inv.site_id === siteId && inv.product_id === prodId);
            return match ? [match] : [];
        }
        return exports.memoryStore.inventory || [];
    }
    if (sql.includes('from material_transfers')) {
        if (sql.includes('where id = $1')) {
            const targetId = p[0];
            const match = (exports.memoryStore.material_transfers || []).filter((t) => t.id === targetId);
            return match;
        }
        return exports.memoryStore.material_transfers || [];
    }
    if (sql.includes('from material_usage')) {
        return exports.memoryStore.material_usage || [];
    }
    if (sql.includes('from damaged_materials')) {
        return exports.memoryStore.damaged_materials || [];
    }
...
async function query(text, params) {
    ...
    if (useInMemoryFallback || !env_js_1.ENV.DATABASE_URL || env_js_1.ENV.DATABASE_URL.includes('[YOUR_PASSWORD]')) {
