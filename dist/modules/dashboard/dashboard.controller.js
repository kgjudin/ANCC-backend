...
async function getAdminDashboard(req, res, next) {
    ...
    try {
        ...
        const totalEmployeesRes = await (0, db_js_1.query)(`SELECT COUNT(*) as cnt FROM employees WHERE (company_id = $1 OR company_id IS NULL OR $1 = '00000000-0000-0000-0000-000000000001') AND status = 'Active'`, [companyId]);
        const presentTodayRes = await (0, db_js_1.query)(`SELECT COUNT(*) as cnt FROM attendance a JOIN employees e ON a.employee_id = e.id WHERE (e.company_id = $1 OR e.company_id IS NULL OR $1 = '00000000-0000-0000-0000-000000000001') AND a.date = $2 AND a.status = 'Present'`, [companyId, today]);
       ...
       ORDER BY fd.created_at DESC LIMIT 5`, [companyId]);
        const recentActivities = await (0, db_js_1.query)(`SELECT al.*, e.full_name as actor_employee_name
       FROM audit_logs al
       LEFT JOIN employees e ON al.actor_employee_id = e.id
       ORDER BY al.timestamp DESC LIMIT 10`, []);
        const fin = financialTotalsRes[0] || {};
        const formattedPurchases = (recentPurchases && recentPurchases.length > 0) ? recentPurchases.map((p) => ({
        ...
        const formattedActivities = (recentActivities && recentActivities.length > 0) ? recentActivities.map((a) => ({
            id: a.id,
