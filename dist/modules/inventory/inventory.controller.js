...
exports.reportDamagedMaterial = reportDamagedMaterial;
exports.createMaterialTransfer = createMaterialTransfer;
...
async function getInventoryOverview(req, res, next) {
    ...
    try {
        ...
        const items = await (0, db_js_1.query)(`SELECT inv.*, st.name as site_name, pr.name as product_name, pr.product_code, pc.name as category_name
       FROM inventory inv
       ...
       ${whereClause}
       ORDER BY st.name ASC, pr.name ASC`, params);
        // Summary calculations
        const totalItems = items ? items.length : 0;
        const lowStockCount = (items || []).filter((i) => Number(i.current_balance || 0) <= Number(i.min_stock_level || 10)).length;
        const totalDamagedCount = (items || []).reduce((acc, i) => acc + (Number(i.damaged_qty) || 0), 0);
        const totalExcessCount = (items || []).reduce((acc, i) => acc + (Number(i.unwanted_qty) || 0), 0);
        return res.json({
            success: true,
            data: {
                items: items || [],
...
async function recordMaterialUsage(req, res, next) {
    const authReq = req;
    try {
        const data = validation_1.MaterialUsageSchema.parse(authReq.body);
        const companyId = authReq.user?.company_id || '00000000-0000-0000-0000-000000000001';
      ...
      RETURNING *`, [
        ...
        await (0, db_js_1.query)(`UPDATE inventory SET used_qty = $1, current_balance = $2, updated_at = NOW() WHERE id = $3`, [newUsed, newBalance, inv.id]);
...
async function reportDamagedMaterial(req, res, next) {
    const authReq = req;
    try {
        const data = validation_1.DamagedMaterialSchema.parse(authReq.body);
        const companyId = authReq.user?.company_id || '00000000-0000-0000-0000-000000000001';
        const actorEmployeeId = authReq.employee?.id;
        const existingInv = await (0, db_js_1.query)(`SELECT * FROM inventory WHERE site_id = $1 AND product_id = $2`, [data.site_id, data.product_id]);
        let inv = existingInv && existingInv.length > 0 ? existingInv[0] : null;
      ...
      RETURNING *`, [
        ...
        if (inv) {
            ...
            await (0, db_js_1.query)(`UPDATE inventory SET damaged_qty = $1, current_balance = $2, updated_at = NOW() WHERE id = $3`, [newDamaged, newBalance, inv.id]);
        }
...
async function createMaterialTransfer(req, res, next) {
    const authReq = req;
    try {
        const data = validation_1.MaterialTransferSchema.parse(authReq.body);
        const companyId = authReq.user?.company_id || '00000000-0000-0000-0000-000000000001';
        const actorEmployeeId = authReq.employee?.id;
        // Sequence TRF-XXXX
        const countRes = await (0, db_js_1.query)(`SELECT COUNT(*) as cnt FROM material_transfers WHERE (company_id = $1 OR company_id IS NULL OR $1 = '00000000-0000-0000-0000-000000000001')`, [companyId]);
...
async function updateMaterialTransferStatus(req, res, next) {
    const authReq = req;
    try {
        ...
        const companyId = authReq.user?.company_id || '00000000-0000-0000-0000-000000000001';
        const actorEmployeeId = authReq.employee?.id;
        const trfRows = await (0, db_js_1.query)(`SELECT * FROM material_transfers WHERE id = $1 AND (company_id = $2 OR company_id IS NULL OR $2 = '00000000-0000-0000-0000-000000000001')`, [id, companyId]);
        ...
        const trf = trfRows[0];
        const updated = await (0, db_js_1.query)(`UPDATE material_transfers SET
        status = $1,
       ...
       RETURNING *`, [status, actorEmployeeId, notes || null, id, companyId]);
        ...
        if (status === 'Received') {
            ...
            const fromInv = await (0, db_js_1.query)(`SELECT * FROM inventory WHERE site_id = $1 AND product_id = $2`, [trf.from_site_id, trf.product_id]);
            if (fromInv && fromInv.length > 0) {
                ...
                await (0, db_js_1.query)(`UPDATE inventory SET transferred_out_qty = $1, current_balance = $2, updated_at = NOW() WHERE id = $3`, [newOut, newBalA, invA.id]);
            }
            // 2. To Site: Increase transferred_in_qty
            const toInv = await (0, db_js_1.query)(`SELECT * FROM inventory WHERE site_id = $1 AND product_id = $2`, [trf.to_site_id, trf.product_id]);
            if (toInv && toInv.length > 0) {
                ...
                await (0, db_js_1.query)(`UPDATE inventory SET transferred_in_qty = $1, current_balance = $2, updated_at = NOW() WHERE id = $3`, [newIn, newBalB, invB.id]);
            }
            else {
...
async function getMaterialTransfers(req, res, next) {
    const authReq = req;
    try {
        const companyId = authReq.user?.company_id || '00000000-0000-0000-0000-000000000001';
        ...
        const transfers = await (0, db_js_1.query)(`SELECT mt.*, s1.name as from_site_name, s2.name as to_site_name, pr.name as product_name, e.full_name as requested_by_name
       ...
       ${whereClause}
       ORDER BY mt.created_at DESC`, params);
        return res.json({
            success: true,
            data: {
                items: transfers || [],
...
async function getInventoryTransactions(req, res, next) {
    ...
    try {
        ...
        const transactions = await (0, db_js_1.query)(`SELECT it.*, st.name as site_name, pr.name as product_name, e.full_name as created_by_name
       ...
       ${whereClause}
       ORDER BY it.created_at DESC`, params);
        return res.json({
            success: true,
            data: {
                items: transactions || [],
