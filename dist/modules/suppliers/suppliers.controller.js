...
exports.checkSupplierDuplicate = checkSupplierDuplicate;
exports.createSupplier = createSupplier;
exports.getSupplierHistory = getSupplierHistory;
...
async function getSuppliers(req, res, next) {
    ...
    try {
        ...
        const countRows = await (0, db_js_1.query)(`SELECT COUNT(*) as total FROM suppliers s ${whereClause}`, params);
        const total = Number(countRows[0].total);
        params.push(Number(limit), offset);
        const suppliers = await (0, db_js_1.query)(`SELECT s.*, e.full_name as registered_by_name
       FROM suppliers s
...
async function checkSupplierDuplicate(req, res, next) {
    const authReq = req;
    try {
        const companyId = authReq.user?.company_id;
        ...
        if (company_name || gst_number) {
            possibleDuplicates = await (0, db_js_1.query)(`SELECT id, supplier_code, company_name, contact_person, phone, gst_number
         FROM suppliers
...
async function createSupplier(req, res, next) {
    const authReq = req;
    try {
        const data = validation_1.SupplierSchema.parse(authReq.body);
        const companyId = authReq.user?.company_id;
        const actorEmployeeId = authReq.employee?.id;
        const existing = await (0, db_js_1.query)(`SELECT id, company_name FROM suppliers 
       WHERE company_id = $1 AND (LOWER(company_name) = LOWER($2) OR (gst_number IS NOT NULL AND gst_number = $3 AND $3 <> ''))`, [companyId, data.company_name, data.gst_number || '']);
        ...
        const countRes = await (0, db_js_1.query)(`SELECT COUNT(*) as cnt FROM suppliers WHERE company_id = $1`, [companyId]);
        const seq = Number(countRes[0].cnt) + 1;
        const supplierCode = `SUP-${String(seq).padStart(5, '0')}`;
        const result = await (0, db_js_1.query)(`INSERT INTO suppliers (
      ...
      RETURNING *`, [
        ...
        await (0, audit_service_js_1.logAudit)({
            ...
            module: constants_1.AUDIT_MODULES.SUPPLIER,
            entityType: 'Supplier',
...
async function getSupplierHistory(req, res, next) {
    const authReq = req;
    try {
        const { id } = authReq.params;
        const companyId = authReq.user?.company_id;
        const supplierRows = await (0, db_js_1.query)(`SELECT s.*, e.full_name as registered_by_name
       FROM suppliers s
       LEFT JOIN employees e ON s.registered_by = e.id
       WHERE s.id = $1 AND s.company_id = $2`, [id, companyId]);
        ...
        const metricsRows = await (0, db_js_1.query)(`SELECT 
       ...
       FROM purchases
       WHERE supplier_id = $1 AND company_id = $2`, [id, companyId]);
        const productsRows = await (0, db_js_1.query)(`SELECT DISTINCT p.id, p.product_code, p.name, p.unit
       FROM products p
       JOIN purchase_items pi ON pi.product_id = p.id
       JOIN purchases pur ON pi.purchase_id = pur.id
       WHERE pur.supplier_id = $1 AND pur.company_id = $2`, [id, companyId]);
        const purchaseHistory = await (0, db_js_1.query)(`SELECT pur.*, e.full_name as created_by_name
       FROM purchases pur
       LEFT JOIN employees e ON pur.created_by = e.id
       WHERE pur.supplier_id = $1 AND pur.company_id = $2
       ORDER BY pur.purchase_date DESC`, [id, companyId]);
